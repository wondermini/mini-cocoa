...
//        let specialSymbolCharacters = CharacterSet.symbols
    if password.filter({$0.isNumber}).count >= 1 && password.filter({$0.isLetter}).count >= 1 && password.filter({$0.isSymbol}).count >= 1 && password.count >= 8 {
        result = 5
    }else if   password.filter({$0.isLetter}).count >= 1 && password.count >= 8{
        result = 4
